/*
   ...
   This tester program is to be compiled with bconsole.h and bconsole.c to test your con functions.
...
int test5(){
  ...
  do{
    ...
    key = console.edit(str,11,30, 20, 60, &offset, &curpos, isTextEditor, readonly,insert);
...
void displayText(int from, int offset, char test[][61]){
  ...
  for(i=from;i<from+10 && i<20;i++){
    if(from + offset >= int(bio::strlen(test[i])))
...
int test4(){
  ...
  if(runtest(4)){
    ...
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 60, &offset, &curpos, 0, 0,insert);
  ...
  if(ok && runtest(9)){
    ...
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 22, &offset, &curpos, 0, 0,insert);
    ok = CheckReturnedKey(ENTER, key);
  ...
  if(ok && runtest(10)){
    ...
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 20, &offset, &curpos, 0, 0,insert);
    ok = CheckReturnedKey(ENTER, key);
  ...
  if(ok && runtest(11)){
    ...
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 50, &offset, &curpos, 0, 0,insert);
    ok = CheckReturnedKey(ENTER, key);
    ok = CheckIoEditVals(0, curpos, 0, insert, 0, offset) && ok;
    ok = CheckStrValue("ABCDEFGHIJKLMNOPQRSTUVWXY", str) && ok;
    pause(ok);
  ...
  if(ok && runtest(20)){
    cls();
    PrnTextBox(1, 5, 70, 8, "Test 4 (continued), console.edit() InTextEditor set to true. Press any key to continue:");
    console.pause();
  }
#endif
  ...
  if(ok && runtest(21)){
    ...
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 60, &offset, &curpos, 1,0,insert);
  ...
  if(ok  && runtest(22)){
    ...
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 50, &offset, &curpos, 1,0,insert);
    ok = CheckReturnedKey(0, key);
    ok = CheckIoEditVals(0, curpos, 0, insert, 0, offset) && ok;
    ok = CheckStrValue("ABCDEFGHIJKLMNOPQRSTUVWXY", str) && ok;
    pause(ok);
  ...
  if(ok && runtest(26)){
    ...
    bio::strcpy(&value[offset + curpos], "NOPQRSTUVWXYZabcdefghijklmnopqrs");
  ...
  if(ok && runtest(27)){
    ...
    insert = 1;
    cls();
    PrnTextBox(1, 5, 70, 8, "Test 4.23: console.edit() Escape when InTextEditor = 0, hit HOME key, then 'a', 'b', 'c' and then ESCAPE to test!");
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 50, &offset, &curpos, 0, 0,insert);
  ...
  if(ok  && runtest(28)){
    ...
    insert = 0;
    cls();
    PrnTextBox(1, 5, 70, 8, "Test 4.24: console.edit() Escape when InTextEditor = 1 , hit right arrow 2 times, then 'a', 'b', 'c' and then ESCAPE to test!");
    PrnBox(10, 29, 22, 3);
    key = console.edit(str,11,30, 20, 50, &offset, &curpos, 1, 0, insert);
    ok = CheckReturnedKey(ESCAPE, key);
    ok = CheckIoEditVals(15, curpos, 0, insert,10, offset) && ok;
...
int getLineLength(const char *src, int maxLen){
  const char* last = int(bio::strlen(src)) > maxLen  ? src + maxLen : src + bio::strlen(src);
